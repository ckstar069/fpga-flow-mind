...
**当前状态**：MVP / Phase 0–6 已完成并 tag `v0.1.0-mvp`。Phase 7 已完成，[`completion review`](docs/planning/phase-7-completion-review.md) active。**Phase 8 已完成**，真实 GUI 桌面验收通过，[`completion review`](docs/planning/phase-8-completion-review.md) active。**Phase 9 详细文档已审核通过并转 `active`；Phase 9 Batch A 编码已完成（Provider 抽象、配置模型、Fake/Mock transport、no-network-by-default 守卫与测试），未接入真实 LLM，未发起真实网络调用；Batch B/C/D/E 尚未开始**。Phase 9~11 overview 仍为 `draft`。
