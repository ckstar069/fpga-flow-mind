...
> MVP（Phase 0–6 / tag `v0.1.0-mvp`）是技术闭环 MVP。下列 Post-MVP 文档中，**Phase 7 详细文档已 `active`**；Phase 7 Batch A/B 已实现并进入审核收口，**Phase 7 Batch C 尚未授权**；Phase 7 overview 与 Phase 8~11 overview 仍为 `draft`。进入任一阶段编码前需先编制该阶段详细文档并审核为 active。
...
2. `docs/planning/phase-7-overview-real-project-quality.md`（Phase 7 overview：真实项目质量补强，draft）
   - Phase 7 详细文档（**均已 active；Batch A/B 已实现并在审核收口，Batch C 未授权**）：
