...
| [`phase-7-real-project-evaluation-model.md`](phase-7-real-project-evaluation-model.md) | `draft` | Phase 7 真实项目质量评估数据模型：RealProjectSample/StageEvaluationTarget/4 类 QualityReport/QualityIssue(+Kind+Severity)/QualityRunSummary/QualityAcceptanceStatus。强调评估产物非审计结论、可追溯、评分仅内部门槛 | Phase 7 编码前必读（当前 draft，未进入编码） |
| [`phase-7-evidence-understanding-quality-design.md`](phase-7-evidence-understanding-quality-design.md) | `draft` | Phase 7 evidence/understanding 质量评估与补强设计：复用 Phase 1~6 能力、5 维度评估探针、10 类 issue、检查方式分层、补强 backlog 与允许/禁止边界 | Phase 7 后端编码前必读（当前 draft，未进入编码） |
...
>
> **Phase 7 设计文档当前为 draft**：`phase-7-real-project-evaluation-model.md` + `phase-7-evidence-understanding-quality-design.md`（均 draft）。Phase 7 详细文档尚未审核转 active，**Phase 7 编码未开始**；转 active 后方允许进入 Phase 7 Batch A 编码。
