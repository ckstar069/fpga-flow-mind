...
| [`phase-9-real-llm-grounding-validation.md`](phase-9-real-llm-grounding-validation.md) | `draft` | Phase 9 真实 LLM 与 grounding 验证：单元/集成（fake provider/mock transport）、可选 `#[ignore]` 真实 smoke、安全回归（凭据不泄露/redacted payload）、真实项目 L0/L4 验收、桌面验收、既有能力零回归、禁止项 rg | Phase 9 验证与验收依据（**draft；Phase 9 编码尚未开始，未接入真实 LLM**） |
