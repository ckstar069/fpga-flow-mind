...
| [`phase-9-real-llm-grounding-validation.md`](phase-9-real-llm-grounding-validation.md) | `active` | Phase 9 真实 LLM 与 grounding 验证：单元/集成（fake provider/mock transport）、可选 `#[ignore]` 真实 smoke、安全回归（凭据不泄露/redacted payload）、真实项目 L0/L4 验收、桌面验收、既有能力零回归、禁止项 rg | Phase 9 验证与验收依据（**active；当前允许进入 Phase 9 Batch A（P9-T01~P9-T02）编码；Phase 9 编码尚未开始，真实 LLM 尚未接入；Batch A 仅允许 Provider 抽象、配置模型、Fake/Mock transport、no-network-by-default 守卫与测试，不得真实联网调用；Batch B/C/D/E 尚未开始**） |
