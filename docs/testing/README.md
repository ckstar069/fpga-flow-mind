...
| [`phase-9-real-llm-grounding-validation.md`](phase-9-real-llm-grounding-validation.md) | `active` | Phase 9 真实 LLM 与 grounding 验证：单元/集成（fake provider/mock transport）、可选 `#[ignore]` 真实 smoke、安全回归（凭据不泄露/redacted payload）、真实项目 L0/L4 验收、桌面验收、既有能力零回归、禁止项 rg | Phase 9 验证与验收依据（active；Batch A/B/C/D 已完成并审核收口；Batch E 自动化/真实项目只读验收已完成；completion review 仍为 draft / pending_desktop_acceptance） |
