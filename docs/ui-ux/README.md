...
| [`phase-9-llm-configuration-and-grounded-qa-view.md`](phase-9-llm-configuration-and-grounded-qa-view.md) | `active` | Phase 9 LLM 配置与 Grounded Q&A 视图：最小 Provider 配置入口、provider=mock/real/degraded 状态可见、配置字段与安全文案、Q&A/understanding 体验、错误/degraded 态、不裁决化、延续 Phase 8 视觉 | Phase 9 前端编码前必读（**active；Phase 9 Batch A 编码已完成并审核收口；Phase 9 Batch C 编码已完成并完成审核收口修复（`GroundingValidator` + citation enforcement，51 个单元测试通过）；**Phase 9 Batch D 编码已完成并进入审核收口**（provider 状态条、配置面板、Understanding/Q&A provider badge、grounding/degraded/unknown 状态展示接入工作台 UI），未接入真实 LLM，未发起真实网络调用；Batch E 尚未开始。Phase 10/11 尚未开始**） |
