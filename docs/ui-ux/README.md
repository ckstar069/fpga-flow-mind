...
| [`phase-9-llm-configuration-and-grounded-qa-view.md`](phase-9-llm-configuration-and-grounded-qa-view.md) | `active` | Phase 9 LLM 配置与 Grounded Q&A 视图：最小 Provider 配置入口、provider=mock/real/degraded 状态可见、配置字段与安全文案、Q&A/understanding 体验、错误/degraded 态、不裁决化、延续 Phase 8 视觉 | Phase 9 前端编码前必读（active；Batch A/B/C/D 已完成并审核收口；Batch E 自动化/真实项目只读验收已完成；completion review 仍为 draft / pending_desktop_acceptance） |
