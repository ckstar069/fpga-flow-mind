...
| [`phase-8-product-workbench-view.md`](phase-8-product-workbench-view.md) | `active` | Phase 8 产品级工作台 UI/UX：AgentScope 风格 9 条设计语言、三段式骨架、深色导航/浅色主区/顶部概览/中部筛选/可展开列表/卡片/蓝色强调、视觉语义系统、空/错误状态、文案规范 | Phase 8 前端编码前必读（active；Phase 8 已完成） |
| [`phase-9-llm-configuration-and-grounded-qa-view.md`](phase-9-llm-configuration-and-grounded-qa-view.md) | `active` | Phase 9 LLM 配置与 Grounded Q&A 视图：最小 Provider 配置入口、provider=mock/real/degraded 状态可见、配置字段与安全文案、Q&A/understanding 体验、错误/degraded 态、不裁决化、延续 Phase 8 视觉 | Phase 9 前端编码前必读（**active；当前允许进入 Phase 9 Batch A（P9-T01~P9-T02）编码；Phase 9 编码尚未开始，真实 LLM 尚未接入；Batch A 仅允许 Provider 抽象、配置模型、Fake/Mock transport、no-network-by-default 守卫与测试，不得真实联网调用；Batch B/C/D/E 尚未开始**） |
