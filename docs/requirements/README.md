...
| [`phase-3-understanding-requirements.md`](phase-3-understanding-requirements.md) | `active` | Phase 3 单阶段结构化理解需求：功能点 IU-001~IU-008、ImplementationUnderstanding、StageSummary、claim/evidence binding、confidence（5 值含 supported）、unknown/gap | Phase 3 设计和实施前必读 |
...
| [`phase-7-real-project-quality-requirements.md`](phase-7-real-project-quality-requirements.md) | `active` | Phase 7 真实项目评估与 evidence/understanding 质量补强需求：RQ-001~RQ-008、真实项目样本覆盖、非目标、退出标准。**Phase 7 Batch A/B 已实现并进入审核收口，Batch C 尚未授权** | Phase 7 设计和实施前必读（active；Batch A/B 已实现，Batch C 未授权） |
