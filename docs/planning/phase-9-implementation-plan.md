...
| 2026-06-19 | Batch C 审核收口修复：修复 `ValidationContext.stage_id` 未用于 citation stage 校验的问题，新增 `CitationCheckResult::StageMismatch` 与 `parse_stage_from_evidence_id`，当 stage 不匹配或 evidence_id 格式无法解析时降级；修复 prompt injection 检测因 citation excerpt 原文含注入文本而误判的问题，`check_content_safety` 仅扫描 `response.content`，不再拼接 excerpt_summary；新增 9 个单元测试，grounding_validator 模块 51 个测试全部通过；`cargo test --lib llm::` 121 passed/1 ignored、`cargo test --lib` 675 passed/2 ignored、`cargo check --tests` 0 warning、`npx tsc --noEmit` 与 `npm run build` 通过；rg 边界检查通过；未接入真实 LLM，未发起真实网络调用；Batch C 审核收口修复完成，Batch D/E 尚未开始。 | Claude |
